...
export function plain(tree, parentPath = '') {
  ...
  tree.forEach((node) => {
    ...
    if (node.type === 'added') {
      plainText.push(
        `Property '${propertyPath}' was added with value: ${formatValues(node.value)}`,
      )
    }
    ...
    if (node.type === 'changed') {
      plainText.push(
        `Property '${propertyPath}' was updated. From ${formatValues(node.value1)} to ${formatValues(node.value2)}`,
      )
    }
